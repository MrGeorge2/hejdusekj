...
type LanguageQuery struct {
	...
	// eager-loading edges.
	withLocalizations *LocalizationQuery
...
func (lq *LanguageQuery) QueryLocalizations() *LocalizationQuery {
	query := &LocalizationQuery{config: lq.config}
...
func (lq *LanguageQuery) Clone() *LanguageQuery {
	...
	return &LanguageQuery{
		...
		withLocalizations: lq.withLocalizations.Clone(),
...
func (lq *LanguageQuery) WithLocalizations(opts ...func(*LocalizationQuery)) *LanguageQuery {
	query := &LocalizationQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
	lq.withLocalizations = query
	return lq
}
...
//		GroupBy(language.FieldLanguageCode).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Language.Query().
//		Select(language.FieldLanguageCode).
//		Scan(ctx, &v)
//
...
func (lq *LanguageQuery) sqlAll(ctx context.Context) ([]*Language, error) {
	...
	if query := lq.withLocalizations; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Language)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Localizations = []*Localization{}
		}
		query.Where(predicate.Localization(func(s *sql.Selector) {
			s.Where(sql.InValues(language.LocalizationsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node, ok := nodeids[fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "language_id" returned %v for node %v`, fk, n.ID)
			}
			node.Edges.Localizations = append(node.Edges.Localizations, n)
		}
	}
